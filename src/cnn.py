...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic_cnn = nn.Sequential(
            layer_init(nn.Conv2d(in_channels=3, out_channels=6,
                      kernel_size=3, padding=1)),
            nn.ReLU(),
            nn.BatchNorm2d(6),

            layer_init(nn.Conv2d(in_channels=6, out_channels=6,
                                 kernel_size=3, padding=1)),
            nn.ReLU(),
            nn.BatchNorm2d(6),

            layer_init(nn.Conv2d(in_channels=6, out_channels=12,
                                 kernel_size=3, padding=1)),
            nn.ReLU(),
            nn.BatchNorm2d(12),

            layer_init(nn.Conv2d(in_channels=12, out_channels=12,
                                 kernel_size=3, padding=1)),
            nn.ReLU(),
            nn.BatchNorm2d(12),

            layer_init(nn.Conv2d(in_channels=12, out_channels=6,
                                 kernel_size=3, padding=1)),
            nn.Flatten()
            )

        self.critic_mlp = nn.Sequential(
            layer_init(nn.Linear(in_features=295, out_features=256)),
            nn.ReLU(),
            nn.BatchNorm1d(256),

            layer_init(nn.Linear(in_features=256, out_features=64)),
            nn.ReLU(),
            nn.BatchNorm1d(64),

            layer_init(nn.Linear(in_features=64, out_features=1)),
        )
        ...
        self.actor_mlp = nn.Sequential(
            ...
            layer_init(nn.Linear(in_features=64, out_features=3)),
        )

    def get_value(self, image, direction):
        return self.critic_mlp(torch.concat((self.critic_cnn(image.permute(0,3,1,2)), direction), dim=1))

    def get_action_and_value(self, image, direction, action=None):
        logits = self.actor_mlp(torch.concat((self.actor_cnn(image.permute(0,3,1,2)), direction), dim=1))
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic_mlp(torch.concat((self.critic_cnn(image.permute(0,3,1,2)), direction), dim=1))
